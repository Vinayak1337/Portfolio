...
class Solver {
	...
	setLossMsg(loss: number) {
		if (loss < 1) return 'This is a perfect result.';
		else if (loss < 5) return 'The is close enough.';
		else if (loss < 15)
...
export default function hexToFilter(hex: string) {
	const rgb = hexToRgb(hex);
	if (rgb.length !== 3) throw new Error('Invalid hex');
